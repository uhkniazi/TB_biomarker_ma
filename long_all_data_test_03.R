...
lData$desc = 'Longs data set for including test set vector - no COV'
save(lData, file='Objects/long_data_set_noCOV.rds')
...
save(rf.fit.1, file='Objects/long.rf.fit.1_noCOV.rds')
...
i = which(dfSamples$Illness1 %in% c('Non-active-sarcoidosis', 'Sarcoidosis', 'ATB'))
fGroups = as.character(dfSamples$Illness1[i])
mDat.2 = mDat[i, cvTopGenes]
test = sample(1:nrow(mDat.2), size = nrow(mDat.2) * 0.3, replace = F)
i = grep('ATB', fGroups)
fGroups[-i] = 'OD'
fGroups = factor(fGroups, levels = c('OD', 'ATB'))

oCV.sar = CCrossValidation.LDA(data.frame(mDat.2[test,]), data.frame(mDat.2[-test,]), fGroups[test], fGroups[-test], 
                               level.predict = 'ATB')
par(p.old)
plot.cv.performance(oCV.sar)

## remove all sarcoids first
i = which(dfSamples$Illness1 %in% c('Non-active-sarcoidosis', 'Sarcoidosis'))
dfSamples.ns = dfSamples[-i,]
mDat.ns = mDat[-i,]

## select a new combination
i = which(dfSamples.ns$Illness1 %in% c('LTBI', 'ATB'))
fGroups = as.character(dfSamples.ns$Illness1[i])
mDat.2 = mDat.ns[i, cvTopGenes]
test = sample(1:nrow(mDat.2), size = nrow(mDat.2) * 0.3, replace = F)
i = grep('ATB', fGroups)
fGroups[-i] = 'OD'
fGroups = factor(fGroups, levels = c('OD', 'ATB'))

oCV.ltbi = CCrossValidation.LDA(data.frame(mDat.2[test,]), data.frame(mDat.2[-test,]), fGroups[test], fGroups[-test], 
                               level.predict = 'ATB')
par(p.old)
plot.cv.performance(oCV.ltbi)

### LTBI, Ctrl vs ATB
i = which(dfSamples.ns$Illness1 %in% c('LTBI', 'Ctrl', 'ATB'))
fGroups = as.character(dfSamples.ns$Illness1[i])
mDat.2 = mDat.ns[i, cvTopGenes]
test = sample(1:nrow(mDat.2), size = nrow(mDat.2) * 0.3, replace = F)
i = grep('ATB', fGroups)
...
oCV.od = CCrossValidation.LDA(data.frame(mDat.2[test,]), data.frame(mDat.2[-test,]), fGroups[test], fGroups[-test], 
                                level.predict = 'ATB')
par(p.old)
plot.cv.performance(oCV.od)

### OD vs ATB
fGroups = as.character(dfSamples.ns$Illness1)
mDat.2 = mDat.ns[, cvTopGenes]
test = sample(1:nrow(mDat.2), size = nrow(mDat.2) * 0.3, replace = F)
i = grep('ATB', fGroups)
fGroups[-i] = 'OD'
fGroups = factor(fGroups, levels = c('OD', 'ATB'))
